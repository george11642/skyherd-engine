...
class TestAgentRegistryConsistency:
    ...
    def test_five_distinct_agent_names(self):
...
class TestAgentMeshSmokeTest:
    ...
    async def test_smoke_test_has_five_agents(self, monkeypatch):
        monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
        mesh = AgentMesh()
        results = await mesh.smoke_test(sdk_client=None)
...
class TestAgentMeshLifecycle:
    async def test_start_and_stop(self, monkeypatch):
        ...
        mesh = AgentMesh()
        await mesh.start()
        assert len(mesh._sessions) == 5
        await mesh.stop()

    async def test_start_creates_five_sessions(self, monkeypatch):
        monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
        mesh = AgentMesh()
        await mesh.start()
        try:
