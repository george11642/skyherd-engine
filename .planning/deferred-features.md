...
| Twilio live-call provisioning (account + paid US number ~$1.15/mo) | 3 | Signup requires real-phone SMS verification + paid resource needs explicit CONFIRM. Full Twilio SMS + voice code path covered by mocked tests in `tests/voice/test_call.py` + `tests/mcp/test_rancher_mcp_sms.py`. Run `docs/TWILIO_PROVISIONING.md` when live voice is needed for video hero shot. | HIGH | 2026-04-23 |
